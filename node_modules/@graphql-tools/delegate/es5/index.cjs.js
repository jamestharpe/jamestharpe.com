...
const AggregateError$1 = _interopDefault(require('@ardatan/aggregate-error'));
...
function mergeExternalObjects(schema, path, typeName, target, sources, selectionSets) {
    ...
    sources.forEach(function (source, index) {
        if (source instanceof graphql.GraphQLError || source === null) {
            ...
            var fieldNodes = utils.collectFields({
                schema: schema,
                variableValues: {},
                fragments: {},
...
function mergeFields(mergedTypeInfo, typeName, object, fieldNodes, sourceSubschemaOrSourceSubschemas, targetSubschemas, context, info) {
    ...
    return containsPromises
        ? Promise.all(resultMap.keys()).then(function (results) {
            return mergeFields(mergedTypeInfo, typeName, mergeExternalObjects(info.schema, graphql.responsePathAsArray(info.path), object.__typename, object, results, Array.from(resultMap.values())), unproxiableFieldNodes, combineSubschemas(sourceSubschemaOrSourceSubschemas, proxiableSubschemas), nonProxiableSubschemas, context, info);
        })
...
function reportUnpathedErrorsViaNull(unpathedErrors) {
    if (unpathedErrors.length) {
        ...
        if (unreportedErrors_1.length) {
            ...
            return graphql.locatedError(combinedError, undefined, unreportedErrors_1[0].path);
        }
    }
    return null;
}
...
var FilterToSchema = /** @class */ (function () {
    ...
    FilterToSchema.prototype.transformRequest = function (originalRequest, delegationContext, _transformationContext) {
        return tslib.__assign(tslib.__assign({}, originalRequest), filterToSchema(delegationContext.targetSchema, originalRequest.document, originalRequest.variables));
    };
...
var CheckResultAndHandleErrors = /** @class */ (function () {
    ...
    CheckResultAndHandleErrors.prototype.transformResult = function (originalResult, delegationContext, _transformationContext) {
        return checkResultAndHandleErrors(originalResult, delegationContext.context != null ? delegationContext.context : {}, delegationContext.info, delegationContext.fieldName, delegationContext.subschema, delegationContext.returnType, delegationContext.skipTypeMerging);
    };
...
function checkResultAndHandleErrors(result, context, info, responseKey, subschema, returnType, skipTypeMerging) {
    ...
    if (returnType === void 0) { returnType = info.returnType; }
    var _a = mergeDataAndErrors(result.data == null ? undefined : result.data[responseKey], result.errors == null ? [] : result.errors, info ? graphql.responsePathAsArray(info.path) : undefined), data = _a.data, unpathedErrors = _a.unpathedErrors;
    return resolveExternalValue(data, unpathedErrors, subschema, context, info, returnType, skipTypeMerging);
}
function mergeDataAndErrors(data, errors, path, index) {
    ...
    if (data == null) {
        ...
        return { data: graphql.locatedError(new AggregateError$1(errors), undefined, path), unpathedErrors: [] };
    }
    if (!errors.length) {
    ...
    Object.keys(errorMap).forEach(function (pathSegment) {
        if (data[pathSegment] !== undefined) {
            var _a = mergeDataAndErrors(data[pathSegment], errorMap[pathSegment], path, index + 1), newData = _a.data, newErrors = _a.unpathedErrors;
...
function delegateRequest(_a) {
    ...
    if (targetOperation === 'query' || targetOperation === 'mutation') {
        var executor = (subschemaConfig === null || subschemaConfig === void 0 ? void 0 : subschemaConfig.executor) || createDefaultExecutor(targetSchema, (subschemaConfig === null || subschemaConfig === void 0 ? void 0 : subschemaConfig.rootValue) || targetRootValue);
...
function validateRequest(targetSchema, document) {
    ...
    if (errors.length > 0) {
        if (errors.length > 1) {
            var combinedError = new AggregateError$1(errors);
            throw combinedError;
        }
        var error = errors[0];
